...
class PF : public Slam
{
  public:
    /// @brief       PF based SLAM for remote / valet parking / gps-denied nav.
    ///
    /// @param [in]  landMarks
    /// @param [in]  wayPoints
    /// @param [out] slam state and covariance
    PF(const Eigen::MatrixXf& landMarks, const Eigen::MatrixXf& wayPoints);
    ~PF() = default;
    ...
    Association_t dataAssociateTable(const Eigen::MatrixXf& Z,
                                     ...
                                     int                    numParticles = 1000) override;
    ...
    float likelihood(const Particle_t&      particle,
                     const Eigen::MatrixXf& Z,
                     const Eigen::VectorXi& idf,
                     const Eigen::MatrixXf& R) override;
